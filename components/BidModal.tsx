...
export function BidModal({ slot, onClose }: Props) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isFinite(parsedAmount) || parsedAmount < slot.min_bid) {
      setError(`Bid at least ${formatMoney(slot.min_bid)} to claim this hour.`);
      return;
    }
    ...
    try {
      ...
      // Hand off to Dodo. Keep the spinner up through the redirect.
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            {slot.bid_count > 0 ? (
              <>
                <p className="mt-2 font-mono text-3xl font-bold text-money">
                  {formatMoney(slot.current_bid)}
                </p>
                <p className="mt-1 text-sm text-neutral-500">
                  Bid {formatMoney(slot.min_bid)} or more to claim this hour
                </p>
              </>
            ) : (
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-neutral-400">For your receipt and outbid alerts.</p>
            </div>
          ...
          <p className="mt-3 text-center text-xs text-neutral-400">
            You&apos;ll be charged exactly your bid. If outbid, we&apos;ll notify you.
          </p>
        </form>
      </div>
    </div>
